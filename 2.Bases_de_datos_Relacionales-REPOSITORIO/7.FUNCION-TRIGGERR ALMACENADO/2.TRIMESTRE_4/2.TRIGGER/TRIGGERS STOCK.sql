...
-- TRIGGER PARA PHP SUMAR STOCK PROD PRODUCCION
...
-- TRIGGER PARA JAVA SUMAR STOCK PROD PRODUCCION
CREATE TRIGGER TR_SUMAR_PROD_PRODUC_AL_INSERTAR
AFTER INSERT ON PRODUCCION
FOR EACH ROW
BEGIN
    IF NEW.ESTADO_PRODUCCION = 'FINALIZADA' THEN
        UPDATE PRODUCTO
        SET CANT_EXIST_PRODUCTO = CANT_EXIST_PRODUCTO + NEW.CANT_PRODUCCION
        WHERE ID_PRODUCTO = NEW.ID_PRODUCTO_FK_PRODUCCION;
    END IF;
